...
const messages = {
  en,
  'zh-Hans': zhHans, // key 不能乱写，查看 uniapp 官网约定
}

const i18n = createI18n({
  locale: uni.getLocale(), // 获取已设置的语言，fallback 语言需要在 manifest.json 中设置
  messages,
...
export function getTemplateByKey(key: string) {
  ...
  const message = (messages as Record<string, Record<string, any>>)[locale] // 拿到某个多语言的所有模板（是一个对象)
 ...
 * @param key 多语言的 key，eg: `introduction`
 * @param {object | undefined} data 需要传递的数据对象，里面的key与多语言字符串对应，eg: `{name:'菲鸽'}`
 * @returns
 */
